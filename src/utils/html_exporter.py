...
def _build_assets_data(results: dict[str, BacktestResult], data_root: str) -> list[dict]:
...
def export_multi_tf_html(
    ...
    for tf, results in all_tf_results.items():
        payload[tf] = _build_assets_data(results, data_root)

    html_content = _get_html_template().replace("{{MULTI_TF_DATA_JSON}}", json.dumps(payload))
